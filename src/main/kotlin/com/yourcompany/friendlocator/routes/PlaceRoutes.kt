...
fun Route.placeRoutes() {
    ...
    route("/places") {
        ...
        /**
         * DELETE /api/places/{id}/simple
         * 장소 삭제 (간편 버전 - ID만으로 삭제)
         */
        delete("/{id}/simple") {
            val placeId = call.parameters["id"] ?: return@delete call.respond(
                HttpStatusCode.BadRequest,
            ...
            val deleted = PlaceRepository.deletePlaceSimple(placeId)
            if (deleted) {
                logger.info("Place simple deleted: $placeId")
                call.respond(HttpStatusCode.OK, mapOf("success" to true, "message" to "삭제되었습니다"))
            } else {
                call.respond(HttpStatusCode.NotFound, mapOf("error" to "장소를 찾을 수 없습니다"))
            }
        ...
        get("/user/{userId}") {
            ...
            val places = PlaceRepository.getPlacesByUserId(userId)
            logger.info("Fetched ${places.size} places for user $userId")
            call.respond(HttpStatusCode.OK, places)
        }

        /**
         * DELETE /api/places/user/{userId}
         * 사용자의 모든 장소 삭제
         */
        delete("/user/{userId}") {
            ...
            val deletedCount = PlaceRepository.deleteAllUserPlaces(userId)
            logger.info("Deleted $deletedCount places for user $userId")
            call.respond(HttpStatusCode.OK, mapOf("success" to true, "deletedCount" to deletedCount))
        }
    }
}
