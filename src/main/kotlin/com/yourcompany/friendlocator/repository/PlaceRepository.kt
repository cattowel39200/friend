...
object PlaceRepository {
     ...
     * 사용자의 장소 목록 조회
     */
    fun getPlacesByUserId(userId: String): List<PlaceResponse> {
        return transaction {
            Places.selectAll().where { Places.userId eq userId }
                .orderBy(Places.createdAt, SortOrder.DESC)
                .map { row ->
                    val photos: List<String> = try {
                        json.decodeFromString(row[Places.photos])
     ...
     * 장소 삭제 (간편 버전 - ID만으로)
     */
    fun deletePlaceSimple(placeId: String): Boolean {
        return transaction {
            val deleted = Places.deleteWhere {
     ...
     * 사용자의 모든 장소 삭제
     */
    fun deleteAllUserPlaces(userId: String): Int {
        return transaction {
            Places.deleteWhere {
                Places.userId eq userId
            }
        }
    }
}
